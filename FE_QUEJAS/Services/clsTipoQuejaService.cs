...
namespace FE_QUEJAS.Services
{
    public class clsTipoQuejaService
    {
        public async Task<List<TipoQueja>> obtenerTiposQueja(string token)
        {
            ...
            HttpResponseMessage respuesta = await httpClient.GetAsync("http://localhost:61342/API/TIPOQUEJA/ObtenerTiposQueja");
        ...
        public async Task<bool> insertarTipoQuejaApi(TipoQueja queja, string token)
        {
            ...
            HttpResponseMessage respuesta = await httpClient.PostAsync("http://localhost:61342/API/TIPOQUEJA/InsertarTipoQueja", new StringContent(json, System.Text.Encoding.UTF8, "application/json"));
            string respuestaJson = await respuesta.Content.ReadAsStringAsync();
        ...
        public async Task<bool> actualizarTipoQuejaApi(TipoQueja queja, string token)
        {
            ...
            httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            string json = JsonConvert.SerializeObject(queja);
            HttpResponseMessage respuesta = await httpClient.PostAsync("http://localhost:61342/API/TIPOQUEJA/ActualizarTipoQueja", new StringContent(json, System.Text.Encoding.UTF8, "application/json"));
            string respuestaJson = await respuesta.Content.ReadAsStringAsync();
            var resultado = JsonConvert.DeserializeObject(respuestaJson);
            if (respuesta.StatusCode == HttpStatusCode.OK)
                resultadoIngreso = true;
        ...
        public async Task<bool> eliminarTipoQueja(TipoQueja queja, string token)
        {
            ...
            httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            string json = JsonConvert.SerializeObject(queja);
            HttpResponseMessage respuesta = await httpClient.PostAsync("http://localhost:61342/API/TIPOQUEJA/EliminarTipoQueja", new StringContent(json, System.Text.Encoding.UTF8, "application/json"));
            string respuestaJson = await respuesta.Content.ReadAsStringAsync();
            var resultado = JsonConvert.DeserializeObject(respuestaJson);
            if (respuesta.StatusCode == HttpStatusCode.OK)
                resultadoIngreso = true;
