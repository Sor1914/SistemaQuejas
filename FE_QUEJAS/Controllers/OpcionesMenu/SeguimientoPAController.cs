...
{
    public class SeguimientoPAController : Controller
    {
        string token = "";
        ...
        public SeguimientoPAController()
        {
            ViewBag.Layout = LayoutMenu;
        }
        public async Task<ActionResult> SeguimientoPA(bool limpiar = true)
        {
            if (limpiar)
            {
            ...
            await Paginacion();
            return PartialView("SeguimientoPA");
        }
        ...
        public async Task<ActionResult> Paginacion(int pagina = 1, int tamanoPagina = 5)
        {
            ...
            {
                ListaQuejas = await _Asignacion.obtenerQuejasPA(token);
        ...
        public async Task<ActionResult> mostrarModalVer(int idEncabezado)
        {
            ...
            return await SeguimientoPA(false);
        }
        ...
        public async Task<ActionResult> resolverQueja(EncabezadoQueja encabezado, int idEncabezado)
        {
            try
            {
                encabezado.Id_Encabezado = idEncabezado;
                encabezado.Id_Estado_Externo = 4;
                encabezado.Id_Estado_Interno = 7;
                token = Request.Cookies["TokenJwt"]?.Value;
                List<EncabezadoQueja> encabezadoQuejaOk = await _Asignacion.actualizarEstadoQueja(encabezado, token);
                if (encabezadoQuejaOk != null)
                {
        ...
        public async Task<ActionResult> guardarRechazoQueja(EncabezadoQueja encabezado, int idEncabezado)
        {
            try
            {
                encabezado.Id_Encabezado = idEncabezado;
                encabezado.Id_Estado_Externo = 4;
                encabezado.Id_Estado_Interno = 6;
                token = Request.Cookies["TokenJwt"]?.Value;
        ...
        private string crearHtmlRechazoCliente(string justificacion)
        {
            string html = "<center> <div style='background: #eeeeee; width:520px; border-radius: 0; padding: 20px;'> <table align='center'> <tbody> <tr> <td colspan='2'> <h3 style='text-align: center;'>Queja Rechazada</h3> <p style='text-align: left;'>" +
                            "Señor cuentahabiente, la atención de su queja no procede {0}" +
                            "</p> <center>  ";
        ...
        private string crearHtmlAsignacionInterno(string queja)
        {
            string html = "<center> <div style='background: #eeeeee; width:520px; border-radius: 0; padding: 20px;'> <table align='center'> <tbody> <tr> <td colspan='2'> <h3 style='text-align: center;'>Queja Asignada</h3> <p style='text-align: left;'>" +
                            "Estimados, El sistema para control de quejas por mal servicio o servicio no conforme les informa que se le asignó la queja No. [{0}] Para su atención tiene un plazo máximo de 5 días hábiles, según normativa vigente.  " +
                            "</p> <center>  ";
